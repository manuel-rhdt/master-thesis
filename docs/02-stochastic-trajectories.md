...
To evaluate the estimation procedure described so far in this chapter, we used it to compute the mutual information for the simple biochemical network introduced in @sec:simple_model. As the initial condition we assumed that the system is in the steady state such that we were able to compare our Monte Carlo estimates with the analytical results from @eq:analytical_mi. We start with a detailed explanation of how we generate the signal trajectories and corresponding response trajectories.

### Simulating Signals and Responses

In the model, the signal dynamics arise from the reactions $\emptyset\xrightarrow{\kappa} S, S\xrightarrow{\lambda}\emptyset$ that describe a simple birth-death process. Therefore, to generate signal trajectories we used the stochastic simulation algorithm (SSA) with the initial copy numbers distributed according to the stationary distribution @2009.Gardiner $$P_S(s_0) = e^{-\kappa/\lambda} \frac{(\kappa/\lambda)^{s_0}}{s_0!}$$
...
- explain why Gaussian approximation is useful
- copy numbers are $\approx 50$ and thus high enough for discretization noise to be less important
- the biochemical is very simple without 
...
- higher $N_X$ leads to less noisy results
- increase of $N_S$ changes the slope slightly, however once large enough, we don't have any more changes
...
Clearly, there is an issue with the computation of the mutual information for trajectories that prevents the accurate estimation using the simple Monte Carlo strategy shown above. In @fig:a we found that 
