...
from gillespie.likelihood import log_p, log_p_average
...
def marginal_likelihood(traj_lengths, signal, response, network):
    ...
    for res, res_chunk in zip(response.to_delayed(), response.chunks[0]):
        level2 = []
        for sig in signal.to_delayed():
            ll = delayed(log_p_average)(traj_lengths, sig, res, network)
            level2.append(
                da.from_delayed(
                    ll, shape=(res_chunk,) + traj_lengths.shape, dtype=np.double
                )
            )
        level1.append(da.stack(level2, axis=1))
...
def simulate_outer(count, batch, length, network, ext_trajectory):
    r = np.empty(len(ext_trajectory), dtype=object)
    for i, s in enumerate(ext_trajectory):
        r[i] = simulate_batched(count, batch, length, network, ext_trajectory=s)
...
def computation_start(conf, path):
    ...
    ce_responses = simulate_outer(
        ce_res_per_signal, batch*4, length, res_network, ce_signals
    )
    ...
    ce = da.stack(ce).mean(axis=1).persist()

    me = -marginal_likelihood(
        traj_lengths, me_signals, me_responses, res_network
    ).persist()
    ...
    np.savez(
        ...
        marginal_entropy=me.compute(),
        conditional_entropy=ce.compute(),
        mutual_information=mi.compute(),
    )
