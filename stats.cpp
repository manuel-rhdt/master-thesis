...
void statsout(void) {
    ...
    for (i = 0; i < sys.num_components; i++) {
        ...
        stats_tp.err = (stats_tp.sumsq - stats_tp.sum * stats_tp.sum) / (double) Xrun[i].acc;
        if (stats_tp.err > 0.)
            stats_tp.err = sqrt(stats_tp.err);
...
void runout(int run) {
    ...
    for (i = 0; i < sys.num_components; i++) {
        ...
        Xrun[i].err = (Xrun[i].sumsq - Xrun[i].sum * Xrun[i].sum) / (double) Xrun[i].acc;
        if (Xrun[i].err > 0.)
            Xrun[i].err = sqrt(Xrun[i].err);
